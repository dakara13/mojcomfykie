...
def _validate_optional_prompt(prompt: str) -> None:
...
def run_grok_imagine_i2v_video(
    ...
    _validate_optional_prompt(prompt)
    ...
    if not isinstance(index, int):
        raise RuntimeError("index must be an integer value.")
    if index < INDEX_MIN or index > INDEX_MAX:
        raise RuntimeError(f"index must be between {INDEX_MIN} and {INDEX_MAX}.")

    has_images = images is not None
    task_id_value = (task_id_ref or "").strip()
    has_task_id = bool(task_id_value)
    ...
    if not has_images and not has_task_id:
        raise RuntimeError("Provide either 'images' or 'task_id' as the source image.")
    ...
    if has_images:
        ...
        if mode == "spicy":
            _log(log, "Spicy mode is not supported with external images; sending mode=normal.")
            input_payload["mode"] = "normal"
        ...
        _log(log, f"Using Grok task_id reference: {task_id_value} index={index}")
        input_payload["task_id"] = task_id_value
